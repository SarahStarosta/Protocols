...
if isempty(fieldnames(S))  % If settings file was an empty struct, populate struct with default settings
    S.GUI.Subject           = 555;
    S.GUI.RewardAmount      = [18]; %ul
    S.GUI.RewardDelay       = 5;
    S.GUI.ChangeOver        = 1;%changeover
    ...
    S.GUI.randomDelay       = 0;
    S.Gui.SupriseValue      = [18 18*0.8^6];
...
%% surprise
Suprisetrials = NaN(1,S.GUI.MaxTrials);
surprise = shuffle(repmat([zeros(1,8) 1 2],1,50)); %10 percent of the cases
    count2=1;
%% Main trial loop
for currentTrial = 1:S.GUI.MaxTrials
    ...
    %% surprise?    
    if round(LeftAmount(currentTrial))== round(18*0.8^2);
        count2 = count2+1;
        Suprisetrials(currentTrial)=surprise(count2);
        if surprise (count2) ~=0
            LeftAmount(currentTrial)= S.Gui.SupriseValue(surprise(count2));
            LeftValveTime(currentTrial) =  GetValveTimes(LeftAmount(currentTrial), [1]);       
            
        else
        end
        
    elseif round(RightAmount(currentTrial))== round(18*0.8^2);
        count2 = count2+1;
        Suprisetrials(currentTrial)=surprise(count2);
        if surprise (count2) ~=0
            RightAmount(currentTrial)= S.Gui.SupriseValue(surprise(count2));
            RightValveTime(currentTrial) = GetValveTimes(RightAmount(currentTrial), [3]);
    ...
    BpodSystem.Data.surprise                        = Suprisetrials;
