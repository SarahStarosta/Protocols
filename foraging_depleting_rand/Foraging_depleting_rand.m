...
if isempty(fieldnames(S))  % If settings file was an empty struct, populate struct with default settings
    ...
    S.GUI.depleft           = 0.7; % 1 means no depletion at all
    S.GUI.depright          = 0.5;
...
BpodNotebook('init');
StartRewardLeft= shuffle(repmat(2:8,1,150));
StartRewardRight= shuffle(repmat(2:8,1,150));
% BpodSystem.Data.StartRewardLeft = StartRewardLeft;
% BpodSystem.Data.StartRewardRight = StartRewardRight;
%% Main trial loop
for currentTrial = 1:S.GUI.MaxTrials
    ...
    l=LeftAmount(currentTrial)
    r=RightAmount(currentTrial)
